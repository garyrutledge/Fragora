...
rounds.each do |round|
    five_sample_users = (1..10).to_a.sort{ rand() - 0.5 }[0..5]
    5.times do
        g = Game.new(:round => round, :user_id => five_sample_users.pop, :deaths => rand(100), :headshots => rand(6)   )
        g.shots_fired = rand(100)
        g.shots_hit = (Random.new.rand(0..(g.shots_fired))) # Random number thats never higher than shots_fired
        g.kill = (Random.new.rand(0..(g.shots_fired))) # Random number thats never higher than shots_fired
        g.save
    end
end
